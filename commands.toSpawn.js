...
//  console.log(around.length);
    for(var e in around) {
        if(around[e].type == 'structure') {
            if((around[e].structure.structureType != STRUCTURE_SPAWN && around[e].structure.structureType != STRUCTURE_LINK
...
static homeAnalyzed(room, spawn) {
    if(!spawn.memory.alphaSpawn) return false;
        ...
        var roads = [[0,1],[-1,0],[0,-1],[1,0]];
        //            spawn.room.createConstructionSite(roads[e][0],roads[e][1], STRUCTURE_ROAD);
            for(var e in roads) {
                spawn.room.createConstructionSite(roads[e][0]+spawn.pos.x,roads[e][1]+spawn.pos.y, STRUCTURE_ROAD);
...
// RawMemory.segments[memory.segmentID]
        ...
        let report = '>>'+spawn.memory.TotalBuild+'/1500'+'('+spawn.memory.totalCreep+')'+'<<'+ spawn.name+ ' RM: ' + spawn.room.name+ '(E:' + spawn.room.energyAvailable + "/"+spawn.room.energyCapacityAvailable +")"
         +'>MD:' + spawn.memory.mode+ ' Bld:'+ getModuleLevel(spawn)
//         'S:'+storage+"/"+terMsg+" T:"+terminal+"/"+stoMsg
        +  '|MD:'+spawn.memory.currentStack+'C:'+spawn.memory.create.length+'W:'+spawn.memory.warCreate.length+'E:'+spawn.memory.expandCreate.length;
...
/*
        ...
        if ( spawn.memory.CreatedMsg != undefined) {
             report2 = spawn.name+" "+spawn.spawning.remainingTime+"/"+spawn.spawning.needTime+" "+  spawn.memory.CreatedMsg ;
