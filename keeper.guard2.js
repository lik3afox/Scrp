...
function getHostiles(creep, range) {
    ...
    if (tgt !== null && tgt.owner.username !== 'Invader') {
        ...
        let bads = creep.pos.inRangeTo(creep.room.invaders, 3);
...
/*    let player = creep.room.enemies; //  NPC
...
/*    let least = _.min(bads, o => o.hits);
    if (creep.pos.isNearTo(least)) {
        ...
        console.log('Does Guard go here?',roomLink(creep.room.name));
        return;
    }*/
...
class roleGuard extends roleParent {
    ...
    static run(creep) {
        ...
        creep.partyFlag.memory.rallyCreateCount = 10;
        ...
        } else if (creep.memory.keeperLair) {
