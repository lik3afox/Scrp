...
var prohibited = ['KO','GH','LO','UO','LH','H','O','G','OH','GO','GH2O','GHO2'
,'XLH2O','XUH2O','XUHO2','XGH2O','XGHO2','XZH2O','XLHO2','XKHO2','XKH2O','XZHO2'];
...
if(lowestStore == undefined ) return false;
		...
		let amount = lowestStore.store[RESOURCE_ENERGY] *.1;
...
if(Memory.termReport )	console.log(terminal,'has ',eTotal,targetRoom);
    ...
    let target = {price:.0001,id:undefined,cost:1000,amount:1,room:undefined,trans: 1};
    let EnergyNum = 100;
...
console.log('profit:',target.price*trans,'#',trans,'EnergyCost',(target.price+target.cost),(target.price+target.cost)*trans,'Profit per Energy',target.price/(1+target.cost)	);
    ...
    if(Memory.sellingHistory == undefined )
    	{ Memory.sellingHistory = [];}
...
if(total < 150000) return;
	...
	for (var e in Orders) {
		if(Orders[e].price < sellingPoint) {
			sellingPoint = Orders[e].price;
		}
	}
	console.log(ORDER_SELL, target.resource, sellingPoint, Math.floor( (target.amount*.1)) , terminal.pos.roomName,orderExist(target.resource,terminal.pos.roomName,sellingPoint));

if(sellingPoint <= .01) return;
if(orderExist(target.resource,terminal.pos.roomName,sellingPoint)) return;
	let whatHappened = Game.market.createOrder(ORDER_SELL, target.resource, sellingPoint, Math.floor( (target.amount*.05)) , terminal.pos.roomName);
...
//if(true)				console.log('Sending Minerals:',target.pos.roomName,mineral,amount, 'azz');
			if(target != undefined ) {
				...
				if(amount == undefined) {
					amount = Math.floor(  target.store[mineral] *.15 );
				}
//					if(amount < 90) return;
...
function sendEnergy(terminal){
	...
	let energyAmount = terminal.store[RESOURCE_ENERGY] * .1;
...
let zz = _.filter(Game.market.orders,function(o) { return o.type == "buy" && o.resourceType == e&&o.active == true }) ;
