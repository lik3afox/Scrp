...
function  showTowerRange(tower) {
    ...
    tower.room.visual.rect(tower.pos.x-20,tower.pos.y-20,40,40,{
        fill: 'transparent', stroke: 'white',lineStyle:'dashed',opacity:.01,fill:color,strokeWidth:.2
    })
    ...
    tower.room.visual.rect(tower.pos.x-5,tower.pos.y-5,10,10,{
        fill: 'transparent',  stroke: 'white',strokeWidth:.2, lineStyle:'dashed',opacity:.01,fill:color
    })
...
//        if(hostiles.length > 0) {
  ...
  //      }
        var hurt = Game.rooms[roomName].find(FIND_MY_CREEPS, {
            filter: (thisCreep) => { return thisCreep.hits < thisCreep.hitsMax;        }
        });
