...
function defendFromInvaders(towers, hostiles) {
    var e = towers.length;
    while (e--) {
...
function doTowerPatterns(towers, hostiles) {
    var e;
    // First lets see if there are weak targets - no healing or below tough damage.
    ...
    //if (room.memory.towerPattern === undefined) {
        room.memory.towerPattern = [
            { pattern: WAIT, timer: 75 },
            { pattern: RANDOMFIRE, timer: 7 },
            { pattern: LOWEST2HP, timer: 1 },
            { pattern: LOWESTHP, timer: 1 },
            { pattern: WAIT, timer: 5 },
            { pattern: RANDOMFIRE, timer: 3 },
            { pattern: LOWEST23HP, timer: 1 },
            { pattern: LOWEST2HP, timer: 1 },
            { pattern: LOWESTHP, timer: 1 },
            { pattern: WAIT, timer: 5 },
            { pattern: RANDOMFIRE, timer: 3 },
    ...
    //}
    if (room.memory.currentPatternCount === undefined) {
        room.memory.currentPatternCount = 0; // This tracks which pattern the array is in currently
    }
    if (room.memory.currentPatternTimer === undefined) {
        room.memory.currentPatternTimer = 0; //This keeps track of how long it's be going on for. if it equsl timer of pattern then next.
    }
    var currentPattern = room.memory.towerPattern[room.memory.currentPatternCount];
    room.memory.currentPatternTimer++;

    if (room.memory.currentPatternTimer >= currentPattern.timer) {
        room.memory.currentPatternTimer = 0;

        room.memory.currentPatternCount++;
        if (room.memory.currentPatternCount >= room.memory.towerPattern.length) {
            room.memory.currentPatternCount = 0;
        }
    }
    //    console.log('Current tower pattern in ', room, currentPattern.pattern, room.memory.currentPatternTimer, currentPattern.timer);
    ...
    switch (currentPattern.pattern) {
        ...
        case RANDOMFIRE:
            if (hostiles.length > 0) {
                ...
                while (e--) {
                    if (towers[e].energy > 0) {
                        towers[e].attack(hostiles[Math.floor(Math.random() * hostiles.length)]);
                    }
                }
            }
            break;
...
class roleTower {
    static roomTower(roomName) {
        ...
        if (roomName === 'E13S18') {
            ...
            hostiles = _.filter(Game.rooms[roomName].find(FIND_HOSTILE_CREEPS), function(o) {
                return !_.contains(fox.friends, o.owner.username);
            });
        }
        var hurt = Game.rooms[roomName].myHurtCreeps;
        scanForRepair(Game.rooms[roomName]); // Occurs eveyr X ticks and adds to repair array
        ...
        if (hostiles.length > 0) {
            if (hostiles[0].owner.username === 'Invader' || Game.rooms[roomName].controller.safeMode) {
                defendFromInvaders(towers, hostiles);
            } else {
                doTowerPatterns(towers, hostiles);
            }
