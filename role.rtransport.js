...
function getSKContainers(creep){
    ...
    for(var e in rooms){
        ...
        if(fs != undefined) {
            fs = _.filter(fs,function(o){return o.structureType == STRUCTURE_CONTAINER});
            for(var a in fs) {
...
let target = Game.getObjectById(creep.memory.repairTarget);
        ...
        zbuild = _.filter(zbuild,function (o) {return o.structureType == STRUCTURE_ROAD && o.hits < o.hitsMax-1500})
        if(zbuild.length > 0 ) {
