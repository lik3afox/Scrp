        ...
        if(doMineralRequest){
            ...
            if(mineralRequest){
                ...
                if(amount > 1500) amount = 1500;
                    console.log(mineralRequest.room,mineralRequest.resourceType,mineralRequest.amount,Memory.stats.totalMinerals[min]);
...
function doInstructions(instructions) {
    ...
    if (instructions.length > 0 && Game.shard.name === 'shard1' && Game.time % 512 === 0) { //
...
//                        console.log(zez.resourceType, current.mineralType, current.order.resourceType,zez.amount);
                }
...
function getMarketPrices() {
    ...
    for (var e in minerals) {
        ...
        if (min === 'token') {
            // We're checking to see if we have one that is existing on this shard.
            let tokenBuy = _.filter(Game.market.orders, function(o) {
                return o.type === ORDER_BUY && o.resourceType === min;
            });

            if (tokenBuy.length > 0) {
                //        let tokenHighBuy = _.max(Game.market.getAllOrders(o => o.type === ORDER_BUY && o.resourceType === min && !Game.market.orders[o.id]), o => o.price);
                //                console.log('Found MY token', tokenBuy[0].price, "and adding info.");
                // Token should already be checked for high price in build.terminal
                rtn[min] = {
                    id: tokenBuy[0].id,
                    buy: tokenBuy[0].price
                };
            } else {
                // If we odn't have one on the market, let's collect the data!
                rtn[min] = {
                    sell: _.max(Game.market.getAllOrders(o => o.type === ORDER_BUY && o.resourceType === min && !Game.market.orders[o.id]), o => o.price)
                };
            }
...
function analyzeMarkets(shardObject) {
    ...
    if (shardObject.empireSettings.market.tier1 && Game.time % 128 === 0) { //
        for (let e in shards) {
            let min = "power";
...
class segmentCommand {
    ...
    static setInterShardData() {
        //        if (Game.time % 32 !== 0) return false;
        ...
        if (Game.shard.name === 'shard1') {
            if (Game.time % (timer + 1) === 0 || rawObject[Game.shard.name] === undefined) {
