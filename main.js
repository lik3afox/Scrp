...
let zz = spawn.pos.findInRange(FIND_CREEPS, 1, {
        filter: object => (object != undefined && object.memory != undefined &&  object != undefined &&!object.memory.reportDeath &&
            object.memory.role != 'scientist' && object.ticksToLive < 1300)
    });    
...
function globalCreep() {
    ...
    if(Memory.scientistCheck < 0) {
        ...
        let sci = _.filter(Game.creeps, function(o) {return o.memory.role == 'scientist' });
        let total = 0;
        total += sci.length;
        for(var e in Game.spawns) {
            if(Game.spawns[e].memory.alphaSpawn) {
                let sci2 = _.filter(Game.spawns[e].memory.create, function(o) {return o.memory.role == 'scientist' });
                if(sci2.length != 0)
...
//    console.log(spawn, spawn.id, 'upgradde', control.level, control.progress, min.mineralAmount);
    if (control.level >= 7 &&  control.progress > 10900000) {
        if (Game.flags['recontrol'] == undefined) {
                ...
                zz = _.filter(zz,function(o){return o.memory.role == 'miner'});
...
if(Game.spawns[title].memory.alphaSpawn) {
                ...
                if ((Game.spawns[title].spawning == undefined)) {
                    if (Game.spawns[title].memory.alphaSpawn) {

let zz = _.filter(Game.spawns,function(o){return o.spawning == undefined && o.room.name == Game.spawns[title].room.name && !o.memory.alphaSpawn})
