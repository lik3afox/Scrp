  ...
  default:
    for(var e in Game.spawns) {
      if(Game.spawns[e].memory.alphaSpawn) {
        ...
        if(tempDis < distance 
          && Game.spawns[e].room.name != 'E35S73' 
          ) {
          distance = tempDis;
          spawn = Game.spawns[e];
        }
      ...
      for(var e in Game.spawns) {
        let spawnz = Game.spawns[e]
            for (var e in spawnz.memory.warCreate) {
                if (currentParty[a][_name] == spawnz.memory.warCreate[e].memory.role && spawnz.memory.warCreate[e].memory.party == flag.name) {
...
if(flag.room != undefined) {
  ...
  if(crps.length > 0) {
    ...
    for(var e in crps) {
      total++;
      if( crps[e].memory.formationPos == undefined)
        crps[e].memory.formationPos = total;
    }
  }
  }
