...
let workparts = _.filter(body,function(n) {return n.type == WORK;}).length;
let moveparts = _.filter(body,function(n) {return n.type == MOVE;}).length;
let attkparts = _.filter(body,function(n) {return n.type == ATTACK;}).length;
let healparts = _.filter(body,function(n) {return n.type == HEAL;}).length;
...
// Game.rooms[roomName].memory.availableMinerals - this hold what is available for creeps to call.
let workparts = _.filter(body,function(n) {return n == WORK;}).length;
let moveparts = _.filter(body,function(n) {return n == MOVE;}).length;
let attkparts = _.filter(body,function(n) {return n == ATTACK;}).length;
let healparts = _.filter(body,function(n) {return n == HEAL;}).length;
let mem = Game.rooms[roomName].memory;
// first update the availableMinearls;
for(var a in mem.labMinerals) {
    mem.availableMinerals[a] =  mem.labMinerals[a] - mem.calledMinerals[a];
